 ...
 * @return array          Layouts, now with Icon Blocks
 */
...
/**
 * Icon Blocks scripts and styles
 */
...
function acfmod_modules_icon_blocks() {
	...
	if ( have_rows( 'icons' ) ) {
		...
		$output .= '<ul class="icon-blocks count-' . esc_attr( $count ) . ' ' . esc_attr( $css_class ) . '">';
		...
		while ( have_rows( 'icons' ) ) :
			...
			if ( ! empty( $link ) ) {
				$output .= '<a href="' . esc_attr( $link ) . '"' . esc_attr( $target ) . '>';
			}
			...
			if ( ! empty( $heading ) ) {
				$heading_color = get_sub_field( 'heading_color' ) ? ' style="color:' . esc_attr( get_sub_field( 'heading_color' ) ) . ';"' : '';
